...
input.addEventListener("input", async () => {
  ...
  try {
    const response = await fetch(
      `https://api.weatherapi.com/v1/search.json?key=d0d50d5c2fad4fdaa6a121250251611&q=${query}`
    );
...
function updateBackground(condition, isDay) {
  ...
  if (weather.includes("rain")) bgImage = "rain.jpg";
    ...
    bgImage = "mist.jpg";
  else if (weather.includes("thunder")) bgImage = "thunder.jpg";
...
async function getWeather(city, isCurrentLocation = false) {
  ...
  try {
    ...
    result.forecast.forecastday.forEach((day) => {
      ...
      card.className =
        "p-3 md:p-4 rounded-lg md:rounded-xl bg-white/10 backdrop-blur-lg text-white text-center shadow-md m-2 w-32";
      card.innerHTML = `
        <p class="md:font-semibold sm:font-normal md:text-sm text-[11px]">${formattedDate}</p>
        ...
        <p class="md:text-xs text-[9px] text-gray-300">Min ${minTemp}°C</p>
      `;
